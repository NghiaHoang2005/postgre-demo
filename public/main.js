...
let currentUser = null; // {username, role, token, id}
let allProducts = [];
let cart = {};
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  loginForm.onsubmit = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        window.location.hash = data.user.role === 'admin' ? '#admin' : '#products';
        window.onhashchange(); // <-- Gọi lại router
        loginUsername.value = loginPassword.value = '';
        loginMsg.textContent = '';
      } else {
        loginMsg.textContent = data.error || data.message || 'Sai tên đăng nhập hoặc mật khẩu!';
      }
    } catch {
  ...
  registerForm.onsubmit = async function(e) {
    ...
    try {
      ...
      registerMsg.textContent = 'Không thể kết nối server!';
    }
  };
